...
class HomeScreen extends React.Component {
  ...
  _retrieveData = async () => {
    try {
      ...
      let c = JSON.parse(o);
      let d = this.state.arraychar;
      console.log(`for loop (${c.length})`)
      for (let i = 0; i < c.length; i++) {
        try {
          d[i].enabled = c[i].enabled
          console.log(`set ${this.state.arraychar[i].enabled} to ${d[i].enabled} for ${d[i].name}`)
        }
        catch (error) {
          alert(error)
        }
  ...
  _storeData = async () => {
    try {
      let o = JSON.stringify(this.state.arraychar)
      AsyncStorage.setItem('somekey', o)
...
class SettingsPage extends React.Component {
  ...
  Alert = () => {
    console.log(this.state.arraychar)
  }
  ...
  render() {
    if (!this.state.isReady) {
      return (
        <AppLoading
          startAsync={this._retrieveData2}
          onFinish={() => this.setState({ isReady: true })}
          onError={console.warn}
        />
      );
    ...
    return (
      <Grid style={{
        ...
        <Button color="tomato" title="Alert" onPress={this.Alert} />
        <Button color="tomato" title="True" onPress={this.SetThing} />
        <Button color="tomato" title="False" onPress={this.SetThingFalse} />
      </Grid>
    )
  }
};
